...
if __name__ == '__main__':
    ...
    from dataset import newCSL_Isolated
    from models.Conv3D import resnet18, resnet34, resnet50, r2plus1d_18
    # Arguments
    parser = argparse.ArgumentParser()
    parser.add_argument('--data_path', default='F:/中国手语数据集科大/testset/0_4',
    ...
    test_set = newCSL_Isolated(data_path=data_path, label_path=label_path, frames=sample_duration,
    ...
    for i in range(len(label_show)):
        print
        # print(f"实际类别：{label_show[i]} {label_list[label_show[i]]}    预测类别：{pred_show[i]} {label_list[pred_show[i]]}")
        print(f"实际类别：{label_list[label_show[i]]}    预测类别：{label_list[pred_show[i]]}")
